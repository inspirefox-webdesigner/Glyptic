...
class ProductLoader {
  ...
  createProductCard(product) {
    ...
    const coverImageContent = product.contents.find(content => content.type === 'coverImage');
    if (coverImageContent && coverImageContent.data) {
      imageUrl = `${this.apiBase.replace('/api', '')}/uploads/${coverImageContent.data}`;
    } else if (product.coverImage) {
      imageUrl = `${this.apiBase.replace('/api', '')}/uploads/${product.coverImage}`;
    } else {
  ...
  renderProductImages(product) {
    ...
    let mainImageSrc = 'assets/img/product/Fire-Alarm-System-fire-Alarm-Control-Panel-addressable-Fire-Detection.jpg';
    let thumbnailImages = [];
 
    // Check for cover image in contents first
    const coverImageContent = product.contents.find(content => content.type === 'coverImage');
    if (coverImageContent && coverImageContent.data) {
      mainImageSrc = `${this.apiBase.replace('/api', '')}/uploads/${coverImageContent.data}`;
    } else if (product.coverImage) {
      mainImageSrc = `${this.apiBase.replace('/api', '')}/uploads/${product.coverImage}`;
    }
 
    // Get variation images from contents
    const variationImagesContent = product.contents.find(content => content.type === 'variationImages');
    ...
    if (thumbnailImages.length > 0) {
      thumbnailImages.forEach((imageName, index) => {
        ...
        thumbnail.style.cssText = 'width: 80px; height: 80px; object-fit: cover; border-radius: 8px; cursor: pointer; border: 2px solid transparent; margin-right: 10px; margin-bottom: 10px;';
