...
class DynamicProductsLoader {
  ...
  createProductCards(product, container) {
    ...
    const coverImageContent = product.contents.find(c => c.type === 'coverImage');
    if (coverImageContent && coverImageContent.data) {
      imageUrl = `http://localhost:5000/uploads/${coverImageContent.data}`;
    } else if (product.coverImage) {
      imageUrl = `http://localhost:5000/uploads/${product.coverImage}`;
    } else {
      ...
      if (firstImage) {
        ...
        imageUrl = `http://localhost:5000/uploads/${imageData}`;
      }
    }
